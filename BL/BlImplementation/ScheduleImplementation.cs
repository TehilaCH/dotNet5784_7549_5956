...
internal class ScheduleImplementation : BlApi.ISchedule
{
    ...
    public DateTime? SetStartProjectDate(DateTime date)=>_dal.Schedule.SetStartProjectDate(date);//A function that writes a date to a file

    public DateTime? SetEndProjectDate(DateTime date) => _dal.Schedule.SetEndProjectDate(date);//A function that writes a date to a file
